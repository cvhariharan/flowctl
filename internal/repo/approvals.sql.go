...
WITH inserted_approval AS (
    ...
    ) RETURNING id, uuid, exec_log_id, action_id, status, decided_by, namespace_id, created_at, updated_at, approvers, approval_groups
)
SELECT
    a.id, a.uuid, a.exec_log_id, a.action_id, a.status, a.decided_by, a.namespace_id, a.created_at, a.updated_at, a.approvers, a.approval_groups,
...
type AddApprovalRequestParams struct {
	...
	ApprovalGroups []string  `db:"approval_groups" json:"approval_groups"`
}
...
func (q *Queries) AddApprovalRequest(ctx context.Context, arg AddApprovalRequestParams) (AddApprovalRequestRow, error) {
	row := q.db.QueryRowContext(ctx, addApprovalRequest,
		...
		arg.ActionID,
		arg.Uuid,
		pq.Array(arg.Approvers),
		pq.Array(arg.ApprovalGroups),
	)
	...
	err := row.Scan(
		...
		&i.CreatedAt,
		&i.UpdatedAt,
		pq.Array(&i.Approvers),
		pq.Array(&i.ApprovalGroups),
		&i.RequestedBy,
	)
	return i, err
}
...
SELECT a.id, a.uuid, a.exec_log_id, a.action_id, a.status, a.decided_by, a.namespace_id, a.created_at, a.updated_at, a.approvers, a.approval_groups FROM approvals a
JOIN execution_log el ON a.exec_log_id = el.id
JOIN flows f ON el.flow_id = f.id
WHERE el.exec_id = $1
  AND a.action_id = $2
...
paged AS (
    SELECT id, uuid, exec_log_id, action_id, status, decided_by, namespace_id, created_at, updated_at, approvers, approval_groups, exec_id, requested_by, flow_name
    FROM filtered
...
SELECT
    p.id, p.uuid, p.exec_log_id, p.action_id, p.status, p.decided_by, p.namespace_id, p.created_at, p.updated_at, p.approvers, p.approval_groups, p.exec_id, p.requested_by, p.flow_name,
    pc.page_count,
    t.total_count
