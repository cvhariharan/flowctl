...
<Aside>Artifact files must be available in the working directory.</Aside>

Artifacts from remote nodes are automatically transferred and made available to subsequent actions at `<NodeName>/path/to/artifact`. If the execution was local, the `<NodeName>` is empty.
...
actions:
  - id: create_on_remote
    ...
    with:
      image: alpine
      script: |
        echo "Hello from remote" > message.txt
    artifacts:
      - message.txt
...
- Learn about [Executors](/general/nodes-and-executors)
