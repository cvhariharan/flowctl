...
<Aside type="note">
The global "superuser" role automatically grants admin access to all namespaces. All other users must be explicitly assigned to namespaces with specific roles.
</Aside>
...
<Aside type="caution">
Admin users can manage all resources within their namespace, including sensitive credentials and secrets. Grant this role carefully.
</Aside>
...
<Aside>
Removing a member from a namespace immediately revokes all their permissions within that namespace. They can no longer view or interact with any resources.
</Aside>
...
<Aside type="caution">
Only the admin user can use username/password authentication. All other users must use OIDC.
</Aside>
